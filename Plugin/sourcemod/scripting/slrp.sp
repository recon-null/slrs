...
{
	// If connection logging is enabled and the client is valid
	if (g_connectionLogging && client > 0 && !IsFakeClient(client))
	{		
		// Holds the timestamp
		...
		new clientSteamQL = strlen(auth) * 2 + 1;
		new clientIPQL = strlen(clientIP) * 2 + 1;
		
		// Quoted client info
		decl String:qClientName[clientNameQL];
		decl String:qClientSteam[clientSteamQL];
		decl String:qClientIP[clientIPQL];
		
		// Quote the client info
		SQL_EscapeString(g_hDatabase, clientName, qClientName, clientNameQL);
		SQL_EscapeString(g_hDatabase, auth, qClientSteam, clientSteamQL);
		SQL_EscapeString(g_hDatabase, clientIP, qClientIP, clientIPQL);
		
		
		// Determine the length of the query
		new queryLen = clientNameQL + clientSteamQL + clientIPQL + sizeof(actionDT) + 256;
		decl String:query[queryLen];

		// Create the query
		Format(query, queryLen, "INSERT INTO tblLogMessages (sid, messageDT, messageType, \
								  userName, userSteam, logLine) \
								 VALUES(%i, %s , 'Connect', '%s', '%s', '%s')",
								 g_sid, actionDT, qClientName, qClientSteam, qClientIP);

		// Send query
		SQL_TQuery(g_hDatabase, T_Generic, query);	
	}	
}
...
LoadSettings()
{
	...
	if (FileExists(locSettings))
	{
		...
		// Load settings		
		g_sid = KvGetNum(kvSettings, "sid");		
		g_connectionLogging = KvGetNum(kvSettings, "connectionLogging");
...
{	
	...
	SQL_EscapeString(g_hDatabase, strSource, qStrSource, strSourceQL);
	...
	if (client > -1)
	{
		...
		// Quote the client info
		SQL_EscapeString(g_hDatabase, clientName, qClientName, clientNameQL);
		SQL_EscapeString(g_hDatabase, clientSteam, qClientSteam, clientSteamQL);
		SQL_EscapeString(g_hDatabase, clientTeam, qClientTeam, clientTeamQL);
		...
		if (target > -1)
		{
			...
			SQL_EscapeString(g_hDatabase, targetName, qTargetName, targetNameQL);
			SQL_EscapeString(g_hDatabase, targetSteam, qTargetSteam, targetSteamQL);
			SQL_EscapeString(g_hDatabase, targetTeam, qTargetTeam, targetTeamQL);
			
			
			// Quote the log message
			new messageQL = strlen(message ) * 2 + 1;			
			decl String:qMessage[messageQL];
			SQL_EscapeString(g_hDatabase, message, qMessage, messageQL);			
			...
			// Create the query
			Format(realQuery, sizeof(realQuery), query, g_sid, actionDT, qClientName, qClientSteam,
				   qClientTeam, qTargetName, qTargetSteam, qTargetTeam, qStrSource, qMessage);
		}
		else
		{
			// Client, no target
			...
			// Create the query
			Format(realQuery, sizeof(realQuery), query, g_sid, actionDT, qClientName, qClientSteam, qClientTeam, qStrSource, qMessage);
		}
	...
	else
	{
		...
		// Create the query
		Format(realQuery, sizeof(realQuery), query, g_sid, actionDT, qStrSource, qMessage);
		
	}	
	
	// Send query
	SQL_TQuery(g_hDatabase, T_Generic, realQuery);
